...
默认 `config.yaml` 中 model_list 为空，所有模型需通过 UI 添加。
...
- Model Name：自定义（如 `claude-sonnet-4-6`）
- LiteLLM Model：选择 Bedrock provider
- Model ID：`bedrock/global.anthropic.claude-sonnet-4-6`
...
| `secrets.tf` | 动态密钥（LiteLLM Master Key、UI 密码） |
