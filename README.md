...
```hcl
# AWS 凭证（可选，默认使用环境变量或 Instance Role）
aws_region  = "us-east-1"
aws_profile = "my-profile"  # 本地开发时使用，CI/CD 留空

# （可选）限制 ALB 访问 IP，默认全开放
allowed_cidr_blocks = ["1.2.3.4/32", "5.6.7.8/32"]

# Bedrock API Keys（用于 claude-sonnet-4-6 负载均衡，必填）
bedrock_api_key_1 = "your-bedrock-api-key-1"
bedrock_api_key_2 = "your-bedrock-api-key-2"
bedrock_api_key_3 = "your-bedrock-api-key-3"
```
...
`claude-sonnet-4-6` 已通过 `config.yaml` 静态配置，部署后直接可用，无需在 UI 中添加。

| model_name | Bedrock Model ID | 路由策略 |
|-----------|-----------------|---------|
| `claude-sonnet-4-6` | `global.anthropic.claude-sonnet-4-6` | simple-shuffle（3 个 API Key 随机轮询） |
...
**Bedrock 模型（无需 API Key）**

Web UI → Models → Add Model：
- Model Name：自定义（如 `claude-haiku`）
- LiteLLM Model：选择 Bedrock provider
- Model ID：`bedrock/anthropic.claude-haiku-4-5-20251001`
...
# 发起请求（请求会被 simple-shuffle 随机路由到 3 个 Bedrock API Key 之一）
...
| `iam.tf` | IAM 角色与权限（含 Bedrock 访问、Secrets Manager 读取） |
| `autoscaling.tf` | Auto Scaling 策略 |
| `secrets.tf` | 动态密钥（LiteLLM Master Key、UI 密码）及 Bedrock API Key（Secrets Manager） |
| `cloudwatch.tf` | CloudWatch 日志组 |
| `config.yaml` | LiteLLM 配置（model_list、router、缓存） |
| `terraform.tfvars.example` | tfvars 模板（复制为 `terraform.tfvars` 后填入实际值） |
| `Dockerfile` | 自定义镜像 |
| `build.sh` | 构建并部署镜像脚本 |
